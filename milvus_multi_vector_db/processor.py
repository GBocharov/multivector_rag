...
from milvus_multi_vector_db.MilvusColbertRetriever import MilvusColbertRetriever, client
from model.ColQwen2ForRAG import get_image_embeddings, get_text_embeddings, processor_generation, device, model

test_retriever = MilvusColbertRetriever(collection_name="test", milvus_client=client)
...
def drop_collection(collection_name:str):
    ...
    collections[collection_name] = MilvusColbertRetriever(collection_name="test", milvus_client=client)
    return f'collection {collection_name} successfully deleted'

def insert_Images(images: List[PIL.Image.Image], names:List[str] = None, collection_name:str = 'test'):
...
def search_Texts( query_list: List[str], collection_name:str = 'test'):
    retriever = collections[collection_name]
    query_list = get_text_embeddings(query_list)
    results = []
